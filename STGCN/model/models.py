from stgcn.model import layers_gat
import torch
import torch.nn as nn

from model import layers, layers_gat
...
class STGCNGraphConv(nn.Module):
    ...
    #  is not the first-order ChebConv, because the renormalization trick is adopted.
...
class STGCNChebGraphConvGAT(nn.Module):
    # STGCNChebGraphConv contains 'TGTND TGTND TNFF' structure
    # ChebGraphConv is the graph convolution from ChebyNet.
    # Using the Chebyshev polynomials of the first kind as a graph filter.
    ...
    def __init__(self, args, blocks, n_vertex):
        super(STGCNChebGraphConvGAT, self).__init__()
        modules = []
        for l in range(len(blocks) - 3):
            modules.append(layers_gat.STConvBlockParallelGAT(args.Kt, args.Ks, n_vertex, blocks[l][-1], blocks[l+1], args.act_func, args.graph_conv_type, args.gso, args.enable_bias, args.droprate, args.num_heads, args.gat_dropout))
        self.st_blocks = nn.Sequential(*modules)
        Ko = args.n_his - (len(blocks) - 3) * 2 * (args.Kt - 1)
        self.Ko = Ko
        if self.Ko > 1:
            self.output = layers_gat.OutputBlock(Ko, blocks[-3][-1], blocks[-2], blocks[-1][0], n_vertex, args.act_func, args.enable_bias, args.droprate)
        elif self.Ko == 0:
            self.fc1 = nn.Linear(in_features=blocks[-3][-1], out_features=blocks[-2][0], bias=args.enable_bias)
            self.fc2 = nn.Linear(in_features=blocks[-2][0], out_features=blocks[-1][0], bias=args.enable_bias)
            self.relu = nn.ReLU()
    ...
    def forward(self, x):
        x = self.st_blocks(x)
         # x: (B, C, T, N) or (B, C, H, W)
         # Ko: output length
         # blocks[-3][-1]: input channels
         # blocks[-2]: output channels
